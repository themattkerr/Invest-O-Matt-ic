...
void PropertySqrftStats::enterDaysOnMarket(int nDOM)
{
    m_nDaysOnMarket = nDOM;
}
...
int PropertySqrftStats::getDaysOnMarket()
{
    return m_nDaysOnMarket;
}
...
{
    if(m_nFinBelowSqrFt > 0)
//        m_dAdjustedPricePerSqrFtBelow = (m_nAdjustedPrice*m_dPercentPriceBelow )/m_nFinBelowSqrFt; // Old way of doing it.  does not account for land value being connected to above grade value.
        m_dAdjustedPricePerSqrFtBelow = m_dPricePerSqrFtBelow;
    else {
...
//        m_dAdjustedPricePerSqrFtAbove = ((1 - m_dPercentPriceBelow)*m_nAdjustedPrice )/m_nFinAboveSqrFt;
        m_dAdjustedPricePerSqrFtAbove = ((static_cast<double>(m_nAdjustedPrice) - static_cast<double>(m_nPriceTotBelow))/ static_cast<double>(m_nFinAboveSqrFt));
    }
    else
